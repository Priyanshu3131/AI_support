...
export const updateTicket = async (req, res) => {
  try {
    ...
    await ticket.save();
    // Add this line to re-fetch and populate
    const populatedTicket = await Ticket.findById(ticket._id).populate("assignedTo", ["name", "email", "_id"]);
...
export const updateTicketStatus = async (req, res) => {
  try {
    ...
    const allowedStatuses = ["TODO", "OPEN", "IN_PROGRESS", "RESOLVED", "CLOSED"];
    ...
    // Add this line to re-fetch and populate
    const populatedTicket = await Ticket.findById(ticket._id).populate("assignedTo", ["name", "email", "_id"]);
    return res.status(200).json({ message: "Ticket status updated", ticket: populatedTicket });
